...
class GurobiLayers(SolverLayer):  
    ...
    def optimize_one(self, value, minimize=True, write=False):
        ...
        if write: self.m.write(f"models/model_{value}_min={minimize}.lp")
        self.m.optimize()
        try:
            return self.m.objVal
        except:
            print("Can't find {bound} bound for value {value}".format(bound="lower" if minimize else "upper", j=j))
            return None
...
class Pipeline():
    def __init__(self, netname, specname, epsilon):
        ...
        nn, self.LB_N0, self.UB_N0 = Pipeline.open_files(netname, specname, epsilon, self.man)
        self.layertypes, self.weights, self.biases = nn.layertypes, nn.weights, nn.biases
    ...
    def get_true_label(netname, specname, man):
        ...
        return np.argmax(
            ElinaLayers(nn.layertypes, nn.weights, nn.biases, man).compute(LB_N0, UB_N0)[1][-1]
        )
