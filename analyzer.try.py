...
class SolverLayer:
    ...
    def __init__(self, layertypes, weights, biases):
        for layertype in layertypes:
            assert layertype in self.SUPPORTED_LT, f"Net type not supported : {layertype}"
        ...
        try:
            ...
            raise Exception(
                "Can't find {bound} bound for value {value}".format(
                    bound="lower" if minimize else "upper", value=value
                )
            )
...
class Pipeline:
    def __init__(self, netname, specname, epsilon):
        ...
        self.nb_hidden_layers = len(self.biases)
    ...
    def get_true_label(netname, specname, man):
        nn, LB_N0, UB_N0 = Pipeline.open_files(netname, specname, 0)
        return np.argmax(
    ...
    def verify(self, lb, ub):
        verified = True

        for j, ub_j in enumerate(ub):
            if (j != self.true_label and lb[self.true_label] <= ub_j):
                verified = False
                break
...
class Strategy2(Pipeline):
    ...
    def compute(self):
        ...
        for i in range(0, self.nb_hidden_layers, self.step):
            j = min(i + self.step, self.nb_hidden_layers)
    ...
    def indexes(self):
        ...
        for i, l in enumerate(map(len, self.biases)):
            if counter >= self.neuron_steps:
                ...
                counter = 0
            counter += l
        idx.append(self.nb_hidden_layers)
        return idx
...
class Strategy4(Pipeline):
    def compute(self):

        lb = [self.LB_N0 if i == 0 else None for i in range(self.nb_hidden_layers+1)]
        ub = [self.UB_N0 if i == 0 else None for i in range(self.nb_hidden_layers+1)]

        lb[1:2], ub[1:2] = ElinaLayers(
            self.layertypes[0:1], self.weights[0:1], self.biases[0:1], self.man
        ).compute(lb[0], ub[0])

        for i in range(1,self.nb_hidden_layers):

            gl = GurobiLayers(
                self.layertypes[:i], self.weights[:i], self.biases[:i], lb[:i], ub[:i]
            )
            #try:
            lb[i], ub[i] = gl.compute(write=True, out=False)
            #except Exception as e:
            #    raise e
            #    print(e)

            lb[i+1:], ub[i+1:] = ElinaLayers(
                self.layertypes[i:], self.weights[i:], self.biases[i:], self.man
            ).compute(lb[i], ub[i])

            if self.verify(lb[-1], ub[-1]):
                return self.true_label, True
                    

        return self.true_label, False
